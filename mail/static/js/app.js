			...
			if (oData && oData.Result && oData.Result && oData.Result.List)
			{
				aList = _.map(oData.Result.List, function (oItem) {
					return oItem && oItem.Email && oItem.Email !== this.owner() ?
						...
						oItem.ForSharedToAll ? {value: oItem.Name, name: oItem.Name, email: oItem.Email} :
...
function CalendarCreateEventPopup()
{
	...
	this.timeOptions = ko.observableArray(this.getDisplayedTimes(AppData.User.defaultTimeFormat() ? 'hh:mm A' : 'HH:mm'));
	AppData.User.defaultTimeFormat.subscribe(function (iTimeFormat) {
		this.timeOptions(this.getDisplayedTimes(AppData.User.defaultTimeFormat() ? 'hh:mm A' : 'HH:mm'));
	}, this);
...
/*CalendarCreateEventPopup.prototype.onHide = function ()
{
 ...
 * @param {Object} oRequest
 */
CalendarCreateEventPopup.prototype.onProcessAppointmentResponse = function (oResponse, oRequest)
{
	if (!oResponse.Result)
	{
...
CalendarCreateEventPopup.prototype.setDayOfWeek = function (iRepeatPeriod)
{
...
CCalendarModel.prototype.expungeEvents = function (aEventIds)
{
	this.events(_.filter(this.events(), function(oEvent) {
		return _.include(aEventIds, oEvent.id);
	}, this));
};
...
function CFileStorageViewModel()
{
	...
	this.rootPath = ko.computed(function () {
		...
		{
			rootPath = Utils.i18n('FILESTORAGE/TAB_SHARED_FILES');
		}
		return rootPath;
	}, this);
...
CFileStorageViewModel.prototype.getGroups = function ()
{
	...
	if (AppData.App.AllowFilesSharing)
	{
		oFolder = new CFileModel();
		oFolder.storageType(Enums.FileStorageType.Shared).displayName(Utils.i18n('FILESTORAGE/TAB_SHARED_FILES')).isFolder(true);
		this.groups.push(oFolder);
	}
};
...
CHelpdeskViewModel.prototype.startAutochekmail = function ()
{
...
{
	var
		aEmails = Utils.isNonEmptyArray(oResponse.Result) ? _.map(oResponse.Result, function (aWatcher) {
			var
