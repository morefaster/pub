...
//					$aResult['VoiceAppSID'] = '';
	...
	public function AppData($bHelpdesk = false, $iHelpdeskIdTenant = null, $sHelpdeskTenantHash = '', $sCalendarPubHash = '', $sFileStoragePubHash = '')
	{
		...
		if ($oApiCapability && $oApiCapability->IsNotLite())
		{
		...
		if ($bHelpdesk)
		{
			...
			if ($bHelpdesk && !$aAppData['Auth'])
			{
				$oApiTenant = /* @var $oApiTenant CApiTenantsManager */ CApi::Manager('tenants');

				$oTenant = $oApiTenant ? (0 < $iIdTenant ? $oApiTenant->GetTenantById($iIdTenant) : $oApiTenant->GetDefaultGlobalTenant()) : null;
				if ($oTenant && $oTenant->HelpdeskStyleAllow)
				{
					$aAppData['HelpdeskStyleImage'] = $oTenant->HelpdeskStyleImage;
				}
			}
