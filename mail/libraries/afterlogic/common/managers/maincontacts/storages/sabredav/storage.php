...
class CApiMaincontactsSabredavStorage extends CApiMaincontactsStorage
{
	...
	protected function getAddressBook($iUserId, $sName)
	{
		...
		if ($this->Init($iUserId))
		{
			...
			{
				$oUserAddressBooks = new \Sabre\CardDAV\UserAddressBooks(
					\afterlogic\DAV\Backends::Carddav(), $this->Principal);
	 ...
	 * @return bool | array
	 */
	public function GetContactItems($iUserId, $iSortField, $iSortOrder, $iOffset, $iRequestLimit, $sSearch, $sFirstCharacter, $iGroupId, $iTenantId)
	{
		$oAddressBook = $this->getAddressBook($iUserId, \afterlogic\DAV\Constants::ADDRESSBOOK_DEFAULT_NAME);
	 ...
	 * @return int
	 */
	public function GetContactItemsCount($iUserId, $sSearch, $sFirstCharacter, $iGroupId, $iTenantId)
	{
		$iCount = 0;
		$oAddressBook = $this->getAddressBook($iUserId, \afterlogic\DAV\Constants::ADDRESSBOOK_DEFAULT_NAME);
